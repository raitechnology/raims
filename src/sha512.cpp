...
ROR64( uint64_t value, int bits ) noexcept
{
  return ( value >> bits ) | ( value << ( 64 - bits ) );
}

static inline size_t
MIN( size_t x, size_t y ) noexcept
{
  return x < y ? x : y;
}

static inline void
LOAD64H( uint64_t &w, const void *buf ) noexcept
{
  const uint8_t *y = (const uint8_t *) buf;
  w                = ( ( uint64_t )( y[ 0 ] & 255 ) << 56 ) |
      ( (uint64_t)( y[ 1 ] & 255 ) << 48 ) |
      ( (uint64_t)( y[ 2 ] & 255 ) << 40 ) |
      ( (uint64_t)( y[ 3 ] & 255 ) << 32 ) |
      ( (uint64_t)( y[ 4 ] & 255 ) << 24 ) |
...
static inline void
STORE64H( uint64_t w, void *buf ) noexcept
{
  uint8_t *y = (uint8_t *) buf;
  y[ 0 ]     = (uint8_t)( ( w >> 56 ) & 255 );
  y[ 1 ]     = (uint8_t)( ( w >> 48 ) & 255 );
  y[ 2 ]     = (uint8_t)( ( w >> 40 ) & 255 );
  y[ 3 ]     = (uint8_t)( ( w >> 32 ) & 255 );
  y[ 4 ]     = (uint8_t)( ( w >> 24 ) & 255 );
  y[ 5 ]     = (uint8_t)( ( w >> 16 ) & 255 );
  y[ 6 ]     = (uint8_t)( ( w >> 8 ) & 255 );
  y[ 7 ]     = (uint8_t)( (w) &255 );
}
...
static void
TransformFunction( Sha512Context &ctx, const uint8_t *buffer ) noexcept
{
  uint64_t S[ 8 ];
  uint64_t W[ 80 ];
  uint64_t t0;
...
void
Sha512Context::initialize( void ) noexcept
{
  this->curlen     = 0;
  this->length     = 0;
...
void
Sha512Context::update( const void *buffer, size_t buflen ) noexcept
{
  size_t n;

  while ( buflen > 0 ) {
    if ( this->curlen == 0 && buflen >= BLOCK_SIZE ) {
      TransformFunction( *this, (const uint8_t *) buffer );
      this->length += BLOCK_SIZE * 8;
      buffer = (const uint8_t *) buffer + BLOCK_SIZE;
      buflen -= BLOCK_SIZE;
    }
    else {
      n = MIN( buflen, ( BLOCK_SIZE - (size_t) this->curlen ) );
      memcpy( this->buf + this->curlen, buffer, n );
      this->curlen += (uint32_t) n;
      buffer = (const uint8_t *) buffer + n;
      buflen -= n;
      if ( this->curlen == BLOCK_SIZE ) {
        TransformFunction( *this, this->buf );
        this->length += 8 * BLOCK_SIZE;
        this->curlen = 0;
      }
    }
  }
...
void
Sha512Context::finalize( void *digest ) noexcept
{
  int i;

  if ( this->curlen >= sizeof( this->buf ) ) {
    return;
  }
  // Increase the length of the message
  this->length += this->curlen * 8ULL;
  // Append the '1' bit
  this->buf[ this->curlen++ ] = (uint8_t) 0x80;
  ...
  // like normal.
  if ( this->curlen > 112 ) {
    while ( this->curlen < 128 ) {
      this->buf[ this->curlen++ ] = (uint8_t) 0;
    }
    TransformFunction( *this, this->buf );
    this->curlen = 0;
  }
  ...
  while ( this->curlen < 120 ) {
    this->buf[ this->curlen++ ] = (uint8_t) 0;
  }
  // Store length
  STORE64H( this->length, this->buf + 120 );
  TransformFunction( *this, this->buf );
  // Copy output
  for ( i = 0; i < 8; i++ ) {
    STORE64H( this->state[ i ], (uint8_t *) digest + ( 8 * i ) );
  }
}
