...
static size_t
uint64_to_bits( uint64_t b,  char *buf ) noexcept
{
  size_t sz = 0;
  uint32_t n = 0;
  while ( b != 0 ) {
    if ( ( b & 1 ) != 0 ) {
      uint32_t d = uint32_digits( n );
      if ( sz + 1 + d < 80 ) {
        if ( sz > 0 ) {
          if ( buf != NULL ) buf[ sz ] = ',';
          sz++;
...
TabPrint::width( Console &console,  char *buf ) noexcept
{
  ...
  switch ( this->type() ) {
    ...
    case PRINT_BITS:
      return uint64_to_bits( this->ival, NULL );
    case PRINT_PERCENT:
...
TabPrint::string( Console &console,  char *buf ) noexcept
{
  ...
  switch ( this->type() ) {
    ...
    case PRINT_LONG:
      ...
      return buf;
    case PRINT_BITS:
      sz = uint64_to_bits( this->ival, buf );
      buf[ sz ] = '\0';
      return buf;
    case PRINT_PERCENT:
                   ...
                   size_t buflen ) noexcept
{
  ...
  switch ( cmd ) {
    ...
    case CMD_SHOW_ROUTES:    this->show_routes( p, int_arg( arg, len ) ); break;
    ...
    case CMD_SHOW_FORWARD:   this->show_forward( p, int_arg( arg, len ) ); break;
...
Console::output_user_route( TabPrint &ptp,  UserRoute &u_rte ) noexcept
{
  ...
  switch ( u_rte.is_set( UCAST_URL_STATE | UCAST_URL_SRC_STATE |
                         MESH_URL_STATE ) ) {
    default: { /* normal tcp */
      uint32_t ucast_fd = u_rte.inbox_fd;
      if ( ucast_fd <= poll.maxfd && poll.sock[ ucast_fd ] != NULL ) {
        uint32_t     uid2;
        bool         found    = false;
...
{
  static const size_t ncols = 9;
  TabOut     out( this->table, this->tmp, ncols );
  TabPrint * tab = NULL;
  uint32_t   count, i = 0, r = 0, sep,
             uid, last_user, last_tport, j;
    ...
    for ( bool ok = rte->uid_connected.first( uid ); ok;
      ...
      if ( ! rte->is_set( TPORT_IS_IPC ) ) {
        for ( j = 0; j < COST_PATH_COUNT; j++ )
          tab[ i++ ].set_int( rte->uid_connected.cost[ j ] );
      }
      while ( i < r+8 )
        tab[ i++ ].set_null();
      if ( uid == rte->uid_connected.rem_uid ) {
        if ( rte->uid_connected.rem_tport_id != 0 )
  ...
  for ( uid = 1; uid < this->user_db.next_uid; uid++ ) {
    ...
    for ( uint32_t t = 0; t < count; t++ ) {
      AdjacencySpace *set = n->adjacency[ t ];
      uint32_t b;
      if ( set == NULL )
        continue;
  ...
  const char *hdr[ ncols ] =
   { "user", "adj", "tport", "type", "cost", "cost2", "cost3", "cost4", "rem" };
  this->print_table( p, hdr, ncols );
    ...
    for ( uint32_t tport_id = 0; tport_id < count; tport_id++ ) {
      ...
      for ( uint8_t path = 0; path < COST_PATH_COUNT; path++ ) {
        if ( n->bloom_rt[ path ] != NULL &&
             n->bloom_rt[ path ]->r == u_ptr->mcast_fd ) {
          b |= 1U << path;
        }
      }
      if ( b == 0 )
...
Console::show_reachable( ConsoleOutput *p ) noexcept
{
  ...
  static const char *hdr[ ncols ] = { "user", "path", "fd", "tport" };
  this->print_table( p, hdr, ncols );
}
